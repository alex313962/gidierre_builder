        ...
        msg.channel.messages.fetch(msg.reference.messageID)
            .then(message => {
                ...
                message.react(react).then(() => {
                    msg.delete()
                    rMsgs.push({ guildID: message.guild.id, channelID: message.channel.id, msgID: message.id, capMem: Number(memberCap) + 1, roleName: nameRole, authorName: msg.author.username })
                });
            })
            .catch(console.error);
